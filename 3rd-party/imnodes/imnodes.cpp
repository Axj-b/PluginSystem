...
void TranslateSelectedNodes(ImNodesEditorContext& editor)
{
    if (GImNodes->LeftMouseDragging)
    {
        ...
        for (int i = 0; i < editor.SelectedNodeIndices.size(); ++i)
        {
            ...
            if (node.Draggable)
            {
                node.Origin += (io.MouseDelta - editor.AutoPanningDelta) / editor.Zoom;
            }
        }
    }
}
...
void DrawGrid(ImNodesEditorContext& editor, const ImVec2& canvas_size)
{
    ...
    const float  scaled_spacing = GImNodes->Style.GridSpacing * editor.Zoom;

    for (float x = fmodf(offset.x, scaled_spacing); x < canvas_size.x; x += scaled_spacing)
    {
    ...
    for (float y = fmodf(offset.y, scaled_spacing); y < canvas_size.y; y += scaled_spacing)
    {
        GImNodes->CanvasDrawList->AddLine(
...
void EndNodeEditor()
{
    ...
    if (GImNodes->AltMouseScrollDelta != 0.f && MouseInCanvas())
    {
        constexpr float zoom_step = 1.1f;
        constexpr float zoom_min  = 0.1f;
        constexpr float zoom_max  = 5.0f;

        const float factor   = GImNodes->AltMouseScrollDelta > 0.f ? zoom_step : 1.0f / zoom_step;
        const float new_zoom = ImClamp(editor.Zoom * factor, zoom_min, zoom_max);

        // Keep the grid point under the cursor stationary during zoom
        const ImVec2 mouse_grid = ScreenSpaceToGridSpace(editor, GImNodes->MousePos);
        editor.Zoom    = new_zoom;
        editor.Panning = GImNodes->MousePos - GImNodes->CanvasOriginScreenSpace - mouse_grid * new_zoom;
    }
